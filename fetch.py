#!/usr/bin/env python
# 2024-11-28 Zhijun Zhang
# Modified based on https://github.com/kad-ecoli/python_scripts/blob/master/fetch.py
docstring='''
fetch 4k5y      Download PDB 4k5y to 4k5y.pdb
fetch 4k5yA     Download PDB 4k5y chain A to 4k5yA.pdb
fetch P34998    Download all PDB associated with uniprot ID P34998
fetch d10mha_   Download SCOPe domain structure d10mha_ to d10mha_.pdb
'''
import os
import re
import sys
...
obsolete_mirror="https://files.wwpdb.org/pub/pdb/data/status/obsolete.dat"
...
class large_split_mapping():
    ...
    def __init__(self,large_split_mapping_file=''):
        ...
        fp=open(large_split_mapping_file,'r')
...
class obsolete_dat(dict):
    ...
    def __repr__(self):
        ...
        return txt
def save(data, out_file_path):
    # 根据data类型决定
    if isinstance(data, bytes):
        out_type = 'wb'
    else:
        out_type = 'w'
    with open(out_file_path, out_type) as file_open:
...
def extract_chain_from_bundle(tarball_name,chainID_list=[]):
    ...
    for Original_chain_ID in chainID_list:
        ...
        pdb_lines=fp.read().decode('utf-8').splitlines()
        fp.close()
...
def extract_chain(PDB_file,chainID_list=[]):
    ...
    for chainID in chainID_list:
        ...
        if not atom_serial_list:
            if re.match(r"^\w+?(\d+)$",chainID):
                chainID,domainID=re.findall(r"^(\w+?)(\d+)$",chainID)[0]
...
def fetch_pdb_list_for_uniprot(accession,all_chain=True):
    ...
    all_chain - True:  (default) all available chains
                False: only the first chain if multiple chains are 
                       present in one PDB 
    '''
    fp=request.urlopen(uniprot_mirror+accession+".txt")
    txt=fp.read().decode('utf-8')
    fp.close()
    ...
    for i,pdbe in enumerate(pdbe_list):
        ...
        for chain in chain_list:
            chain_dict[chain]=re.findall(chain+r"[/\w]*?=(\d+\-\d+)",pdbe[1])
...
def fetch_pdb_sequence(PDBid):
    ...
    fp=request.urlopen(rcsb_fasta_mirror+PDBid)
    txt=fp.read().decode('utf-8')
    fp.close()
    return txt
...
def fetch(arg,outfmt=''):
    ...
    if outfmt=="pdb" or not outfmt:
        if pdb_pattern.match(arg):
            txt=fetch_pdb(arg,include_model) # 从rcsb下载pdb文件
        elif scop_pattern.match(arg):
            txt=fetch_scope(arg) # 从scop下载pdb文件
        elif pdb_chain_pattern.match(arg): # 输出chain pdb
            txt='\n'.join(fetch_chain(arg,include_model))
        elif accession_pattern.match(arg) or entry_pattern.match(arg): # 从uniprot下载pdb文件
            txt='\n'.join(fetch_uniprot(arg, all_chain=True))
        ...
        save(txt, outdir+'/'+arg+'.txt')
            
    elif outfmt=="fasta":
        if accession_pattern.match(arg) or entry_pattern.match(arg):
           txt=fetch_uniprot_sequence(arg) # 从uniprot下载fasta序列
        elif pdb_pattern.match(arg):
           txt=fetch_pdb_sequence(arg) # 从rcsb下载fasta序列
        elif pdb_chain_pattern.match(arg):
           txt=fetch_pdb_chain_sequence(arg) # 从rcsb下载fasta序列
        save(txt, outdir+'/'+arg+'.fasta')
    
    return txt

if __name__=="__main__":
    # default setting
    parser = argparse.ArgumentParser(description="fetch PDB files from various databases.\n\nExample:\n"+docstring,formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("-i","--input", type=str, nargs='+', help="Input entries")
    parser.add_argument("-of","--outfmt", type=str, choices=["mmcif", "pdbx", "cif", "pdb","fasta","list"],default='pdb', help="Output format, can be 'mmcif', 'pdbx', 'cif', 'pdb', 'fasta', or 'list'.")
    parser.add_argument("-im","--include_model", type=str, choices=["true", "false"], default="false", help="Whether download obsolete PDB such as theoretical model (true/false).")
    parser.add_argument("-o","--output", type=str, default="./", help="Output directory.")
    args = parser.parse_args()
    include_model = (args.include_model == "true")
    if args.outfmt in ["mmcif","pdbx"]:
        outfmt="cif"
    else:
        outfmt=args.outfmt
